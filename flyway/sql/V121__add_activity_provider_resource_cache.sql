...
CREATE TABLE activity_provider_resources (
  ...
  provider_version char(64) NOT NULL CHECK (provider_version ~ '^[0-9a-f]{64}  fetched_at timestamptz NOT NULL DEFAULT now(),
...
END $$;
  ...
  CONSTRAINT uq_activity_provider_resources_owner_id UNIQUE (owner_id, id),
  CONSTRAINT uq_activity_provider_resources_resource
    UNIQUE (owner_id, activity_id, provider, resource_type)
);

CREATE INDEX idx_activity_provider_resources_owner_activity
  ON activity_provider_resources (owner_id, activity_id, provider);
