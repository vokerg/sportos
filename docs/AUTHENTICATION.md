...
|---|---|---|
| `sportos_app` | NestJS API, identity provisioning, and session persistence | account-owned work reserves one pooled connection, sets `sportos.account_id` for that connection, and clears it before release; auth control-plane tables are app-only |
...
8. Validate same-user access, denied cross-user access, account-context cleanup, and that the dispatcher cannot read canonical or auth tables.
