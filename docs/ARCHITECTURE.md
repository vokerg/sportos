...
The API authenticates an opaque server-side session, validates CSRF for unsafe methods, reserves one pooled connection, establishes the authenticated account on that connection, runs repository-owned transactions, and clears the context before release. Angular renders API truth and explicitly separates generated guidance from official records. Its authenticated shell lazy-loads one route per major workflow. The Daily Log retains the dense canonical table and adds a Quick entry mode backed by the same owner-scoped manual-save scoring transaction; `/monthly-stats` provides deterministic calendar-bucket comparisons, `/dynamics` provides true calendar-day trailing-window movement with explicit coverage, a reusable selected-day sheet provides bounded highlights, and `/daily/:date` owns the exhaustive day read/edit/provenance workflow without duplicating scoring logic in the browser. `/activities` and `/activity/:id` read canonical activity facts directly through a separate account-scoped repository; they do not read the day ledger or Run Lab performance events. See [Activities](ACTIVITIES.md).
...
- V121 adds the canonical `track` run subtype so Strava TrackRun activities remain filterable without changing scoring authority.
...
Repositories provide narrow boundaries for daily score explanations, bounded cockpit summaries, account-scoped longitudinal facts, performance/provenance reads, rule workflows, import/provider diagnostics, and strict canonical export. Monthly and rolling aggregation treat absent rows as missing coverage and run in deterministic application code. Provider credentials and raw payloads are never part of public read models or canonical export.
