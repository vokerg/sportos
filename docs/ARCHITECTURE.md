...
Repositories provide narrow boundaries for daily score explanations, bounded cockpit summaries, account-scoped dynamics facts, performance/provenance reads, rule workflows, import/provider diagnostics, and strict canonical export. Dynamics aggregation treats absent rows as missing coverage and runs in deterministic application code. Provider credentials and raw payloads are never part of public read models or canonical export.
