...
Some Strava fields may be present only in retained raw source data, and some list responses do not include every optional metric. Cadence, splits, power, GPS tracks, and training load are outside the canonical Activities model and remain outside this MVP. The UI does not infer or display those values from raw provider JSON.
