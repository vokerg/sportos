...
`activity_provider_resources` currently resides in the primary database so existing account-context/RLS guarantees apply. `.env.example` reserves dedicated activity-detail database and Flyway settings for the planned high-volume storage split; wiring and migration of this cache to that database are intentionally deferred.
