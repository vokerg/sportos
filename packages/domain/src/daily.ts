...
export function aggregateActivitiesToDailyFacts(metricDate: string, activities: ActivityFact[], excelAllPoints?: number): DailyMetricFacts {
  ...
  return {
    ...
    runIndoorM: sum(onDate.filter((a) => a.activityType === 'run' && a.subtype === 'treadmill').map((a) => a.distanceM ?? 0)),
    runOutdoorM: sum(onDate.filter((a) => a.activityType === 'run' && a.subtype === 'outdoor').map((a) => a.distanceM ?? 0)),
    runUnspecifiedM: sum(onDate.filter((a) => a.activityType === 'run' && a.subtype === 'unknown').map((a) => a.distanceM ?? 0)),
    bikeM: sum(onDate.filter((a) => a.activityType === 'bike').map((a) => a.distanceM ?? 0)),
    bikeIndoorM: sum(onDate.filter((a) => a.activityType === 'bike' && a.subtype === 'indoor').map((a) => a.distanceM ?? 0)),
    bikeOutdoorM: sum(onDate.filter((a) => a.activityType === 'bike' && a.subtype === 'outdoor').map((a) => a.distanceM ?? 0)),
