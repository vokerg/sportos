...
export class StravaAdapter implements ProviderAdapter {
  ...
  async refreshAuthorization(input: ProviderAuthorization): Promise<ProviderAuthorization> {
    ...
    return { ...refreshed, providerAccountId: input.providerAccountId, displayName: refreshed.displayName ?? input.displayName, scopes: input.scopes };
  }
  ...
  private async tokenRequest(fields: Record<string, string>): Promise<ProviderAuthorization> {
    const response = await this.transport.request({ method: 'POST', url: new URL('/oauth/token', this.authorizationBaseUrl), headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams(fields).toString() });
    success(response, 'Strava authorization failed.');
    const body = record(response.body, 'authorization response');
    const athlete = record(body.athlete, 'athlete');
    return {
...
export class FetchProviderTransport implements ProviderHttpTransport {
  async request(input: HttpRequest): Promise<HttpResponse> {
    ...
    try {
      response = await fetch(input.url, { method: input.method, headers: input.headers, body: input.body, redirect: 'error', signal: AbortSignal.timeout(30_000) });
    } catch { throw new ProviderError('PROVIDER_UNAVAILABLE', 'The provider could not be reached.', true); }
    const text = await response.text();
    let body: unknown = null;
    if (text) { try { body = JSON.parse(text) as unknown; } catch { body = text.slice(0, 500); } }
...
export function stravaActivityFingerprint(activity: ProviderActivity): string {
  return createHash('sha256').update(JSON.stringify({ version: 1, type: canonicalActivityType(activity), startDate: activity.startDate.toISOString(), distanceM: rounded(activity.distanceM), movingTimeS: rounded(activity.movingTimeS ?? activity.elapsedTimeS) })).digest('hex');
}
...
function parseActivity(raw: Record<string, unknown>): ProviderActivity {
  ...
  return {
    providerActivityId: providerId(raw.id, 'activity id'), providerUpdatedAt: optionalDate(raw.updated_at), name: optionalText(raw.name, 500),
    type: requiredText(raw.type, 100, 'activity type'), sportType: optionalText(raw.sport_type, 100), startDate, localDate,
    timezone: optionalText(raw.timezone, 200), distanceM: nonNegative(raw.distance, 'distance'), elapsedTimeS: nonNegative(raw.elapsed_time, 'elapsed time'),
    movingTimeS: nonNegative(raw.moving_time, 'moving time'), elevationGainM: nonNegative(raw.total_elevation_gain, 'elevation gain'),
    averageHeartrate: nonNegative(raw.average_heartrate, 'average heart rate'), maxHeartrate: nonNegative(raw.max_heartrate, 'maximum heart rate'),
    averageSpeedMps: nonNegative(raw.average_speed, 'average speed'), calories: nonNegative(raw.calories, 'calories'),
    isManual: raw.manual === true, isIndoor: raw.trainer === true || raw.indoor === true, isPrivate: raw.private === true, isRace: raw.workout_type === 1, raw,
  };
}
function success(response: HttpResponse, message: string): void { if (response.status < 200 || response.status >= 300) throw responseError(response, message); }
...
function nextQuarter(): Date { const date = new Date(); date.setUTCSeconds(0, 0); date.setUTCMinutes(Math.floor(date.getUTCMinutes() / 15) * 15 + 15); return date; }
...
function parseScopes(value: unknown): string[] { if (Array.isArray(value)) return scopes(value.filter((v): v is string => typeof v === 'string')); if (typeof value === 'string') return scopes(value.split(',')); return []; }
function scopes(values: string[]): string[] { const result = [...new Set(values.map((v) => requiredText(v, 100, 'scope').toLowerCase()))].sort(); if (!result.length) throw new ProviderError('PROVIDER_SCOPE_MISSING', 'At least one provider scope is required.', false); return result; }
...
function requiredText(value: unknown, max: number, name: string): string { if (typeof value !== 'string' || !value.trim() || value.trim().length > max) throw new ProviderError('PROVIDER_RESPONSE_INVALID', `Invalid ${name}.`, false); return value.trim(); }
function optionalText(value: unknown, max: number): string | null { return value === null || value === undefined || value === '' ? null : requiredText(value, max, 'provider text'); }
...
function integer(value: number, min: number, max: number, name: string): number { if (!Number.isInteger(value) || value < min || value > max) throw new ProviderError('PROVIDER_RESPONSE_INVALID', `Invalid ${name}.`, false); return value; }
