...
import {
  ...
  stravaActivityFingerprint,
  type ProviderActivity,
  type ProviderAdapter,
  type ProviderAuthorization,
} from '@sportos/providers';

export interface ProviderSyncRunnerOptions {
  workerId: string;
  leaseSeconds?: number;
...
export class ProviderSyncRunner {
  ...
  async process(job: DispatchedProviderSync): Promise<void> {
    ...
    try {
      ...
      if (authorization.expiresAt.getTime() <= Date.now() + 5 * 60 * 1000) {
        ...
        await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).replaceCredential(
          job.connectionId,
          envelope,
      ...
      let page = cursorPage(job.cursor);
      ...
      for (; page <= 100_000; page += 1) {
        if (await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).cancellationRequested(job.id, this.workerId))) {
          await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).markCancelled(job.id, this.workerId));
          return;
        }
        await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).heartbeat(
          job.id,
          this.workerId,
          `fetching-page-${page}`,
          Math.min(90, 10 + page),
        ...
        let fetched;
        try {
          fetched = await this.adapter.fetchActivityPage({
            authorization,
            page,
            perPage: this.pageSize,
            after: job.requestedAfter ?? undefined,
            before: job.requestedBefore ?? undefined,
          });
        } catch (error) {
        ...
        for (const activity of fetched.activities) {
          ...
          if (!canonicalType) {
            await this.recordSkipped(job.ownerId, batchId, activity, raw, rawHash, 'UNSUPPORTED_ACTIVITY_TYPE');
            warningCount += 1;
            continue;
          }
          const result = await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).ingestActivitySnapshot({
            batchId: requireBatch(batchId),
            connectionId: job.connectionId,
            providerActivityId: activity.providerActivityId,
            providerUpdatedAt: activity.providerUpdatedAt,
            identityFingerprint: stravaActivityFingerprint(activity),
            rawHash,
            raw,
            activity: {
              activityDate: activity.localDate,
              startTime: activity.startDate,
              activityType: canonicalType,
              subtype: subtype(activity),
              distanceM: finiteOrNull(activity.distanceM),
              durationS: finiteOrNull(activity.elapsedTimeS),
              movingTimeS: finiteOrNull(activity.movingTimeS),
              calories: finiteOrNull(activity.calories),
              avgHr: finiteOrNull(activity.averageHeartrate),
              maxHr: finiteOrNull(activity.maxHeartrate),
              elevationGainM: finiteOrNull(activity.elevationGainM),
              avgSpeedMps: finiteOrNull(activity.averageSpeedMps),
              avgPaceSPerKm: pace(activity),
        ...
        await this.withOwner(job.ownerId, async (db) => {
          await new ImportsRepository(db).updateBatchCounts(requireBatch(batchId), {
            row_count: rawCount,
            normalized_count: activityCount,
            warning_count: warningCount,
            status: 'normalized',
          }, 'provider-page-committed');
          await new ProvidersRepository(db).updateCursor(
            job.id,
            this.workerId,
            { page: page + 1, highWatermark: highWatermark?.toISOString() ?? null },
            Math.min(95, 10 + page),
          );
        });
      }
      ...
      await this.withOwner(job.ownerId, async (db) => {
        ...
        await new ProvidersRepository(db).markSucceeded(job.id, this.workerId, {
          rawRecords: rawCount,
          activities: activityCount,
          performanceEvents: performanceCount,
          warnings: warningCount,
          pages: Math.max(0, page - cursorPage(job.cursor)),
        }, highWatermark);
      });
    } catch (error) {
      ...
      if (providerError?.code === 'PROVIDER_REAUTHORIZATION_REQUIRED') {
        await this.withOwner(job.ownerId, (db) => new ProvidersRepository(db).markReauthorizationRequired(job.connectionId, code, message)).catch(() => undefined);
      }
      if (batchId) {
        await this.withOwner(job.ownerId, (db) => new ImportsRepository(db).markBatchFailed(batchId!, {
          phase: 'provider-sync',
    ...
    return this.withOwner(job.ownerId, async (db) => {
      const batch = await new ImportsRepository(db).createBatch({
        ...
        metadata: {
          provider: 'strava',
          connectionId: job.connectionId,
          providerSyncJobId: job.id,
          mode: job.mode,
          requestedAfter: job.requestedAfter?.toISOString() ?? null,
          requestedBefore: job.requestedBefore?.toISOString() ?? null,
        },
      });
  ...
  private async recordSkipped(
    ...
    await this.withOwner(ownerId, async (db) => {
      const records: NewSourceRecord[] = [{
        import_batch_id: batchId,
        source: 'strava_api',
        sheet_name: null,
        row_index: null,
        source_record_key: activity.providerActivityId,
        row_hash: rawHash,
        raw_json: raw,
        ...
        status: 'skipped',
        errors: [],
        warnings: [{ code, message: `Unsupported Strava activity type ${activity.sportType ?? activity.type}.` }],
      }];
      await new ImportsRepository(db).insertSourceRecords(records);
    });
  }

  private withOwner<T>(ownerId: string, callback: (db: Kysely<Database>) => Promise<T>): Promise<T> {
    return withAccountContext(this.dataDb, ownerId, callback);
  }
}

function cursorPage(cursor: Json): number {
  if (typeof cursor !== 'object' || cursor === null || Array.isArray(cursor)) return 1;
  const value = cursor.page;
  return typeof value === 'number' && Number.isInteger(value) && value >= 1 ? Math.min(value, 100_000) : 1;
}

function subtype(activity: ProviderActivity): 'outdoor' | 'indoor' | 'treadmill' | 'manual' | 'race' | 'unknown' {
  if (activity.isManual) return 'manual';
  if (activity.isRace) return 'race';
  if (activity.isIndoor && canonicalActivityType(activity) === 'run') return 'treadmill';
  if (activity.isIndoor) return 'indoor';
  return 'outdoor';
}

function pace(activity: ProviderActivity): number | null {
  const distance = activity.distanceM;
  const duration = activity.movingTimeS ?? activity.elapsedTimeS;
...
function boundedRaw(value: Record<string, unknown>): Json {
  const serialized = JSON.stringify(value);
  if (serialized.length <= 256_000) return JSON.parse(serialized) as Json;
  const safe: Record<string, unknown> = { ...value };
  delete safe.map;
  delete safe.polyline;
  delete safe.summary_polyline;
  delete safe.start_latlng;
  delete safe.end_latlng;
  const reduced = JSON.stringify(safe);
  if (reduced.length > 256_000) {
    return { id: String(value.id ?? ''), truncated: true };
  }
  return JSON.parse(reduced) as Json;
}

function hashJson(value: Json): string {
...
function requireBatch(value: string | null): string {
  if (!value) throw new Error('Provider sync batch is unavailable.');
  return value;
}

function safeWorkerId(value: string): string {
  return value.trim().slice(0, 200) || 'sportos-provider-worker';
}
