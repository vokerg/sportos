...
import { DynamicsPageComponent } from './dynamics-page.component';
...
describe('DynamicsPageComponent', () => {
  it('loads URL-backed controls and routes an updated selection', () => {
    ...
    const api = { dynamics: vi.fn().mockReturnValue(of(response)) };
    const router = { navigate: vi.fn().mockResolvedValue(true) };
    const component = new DynamicsPageComponent(api as unknown as ApiService, { queryParamMap: params } as unknown as ActivatedRoute, router as unknown as Router);

    component.ngOnInit();
    expect(api.dynamics).toHaveBeenCalledWith({ from: '2026-01-01', to: '2026-02-28', granularity: 'monthly', metrics: ['score', 'run'] });
  ...
  it('enforces one-to-four selected metrics and surfaces API failures', () => {
    ...
    const api = { dynamics: vi.fn().mockReturnValue(throwError(() => new Error('offline'))) };
    const component = new DynamicsPageComponent(api as unknown as ApiService, { queryParamMap: params } as unknown as ActivatedRoute, { navigate: vi.fn() } as unknown as Router);
    component.ngOnInit();
