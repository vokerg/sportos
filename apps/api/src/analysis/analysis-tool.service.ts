...
export class AnalysisToolService {
  ...
  execute(request: AnalysisToolRequest, accountId: string): Promise<AnalysisToolResult> {
    switch (request.tool) {
      case 'daily_summary':
        return this.dailySummary(request.input, accountId);
  ...
  private async dailySummary(
    ...
    accountId: string,
  ): Promise<AnalysisToolEnvelope<DailySummaryFacts, 'daily_summary'>> {
    const rows = await this.dailyService.summary(input, accountId);
    const flags: AnalysisDataQualityFlag[] = [];
    if (rows.length === 0) flags.push('NO_DATA');
    if (inclusiveSpan(input.from, input.to) > input.limit) flags.push('RANGE_EXCEEDS_RESULT_LIMIT');

    const days: DailySummaryFact[] = rows.map((row) => ({
    ...
    return envelope('daily_summary', {
      ...
      key: `daily_metric:${day.date}`,
      kind: 'daily_metric' as const,
      date: day.date,
      label: `Official daily metric for ${day.date}`,
    })), quality(flags));
  }
  ...
  private async dailyScoreBreakdown(
    date: string,
    accountId: string,
  ): Promise<AnalysisToolEnvelope<DailyScoreBreakdownFacts | null, 'daily_score_breakdown'>> {
...
function calculateStatistics(days: DailySummaryFact[]): DailySummaryFacts['statistics'] {
  const chronological = [...days].sort((left, right) => left.date.localeCompare(right.date));
  if (chronological.length === 0) {
    return {
      recordCount: 0,
      totalOfficialPoints: 0,
      averageOfficialPoints: null,
      minimum: null,
      maximum: null,
      first: null,
      last: null,
  ...
  const total = chronological.reduce((sum, day) => sum + day.score.officialTotal, 0);
  const minimum = chronological.reduce((best, day) =>
    day.score.officialTotal < best.score.officialTotal ? day : best);
  const maximum = chronological.reduce((best, day) =>
    day.score.officialTotal > best.score.officialTotal ? day : best);
  const first = chronological[0]!;
  const last = chronological[chronological.length - 1]!;
  return {
    recordCount: chronological.length,
    totalOfficialPoints: round(total, 3),
    averageOfficialPoints: round(total / chronological.length, 3),
    minimum: { date: minimum.date, officialTotal: minimum.score.officialTotal },
    maximum: { date: maximum.date, officialTotal: maximum.score.officialTotal },
    first: { date: first.date, officialTotal: first.score.officialTotal },
    last: { date: last.date, officialTotal: last.score.officialTotal },
    firstToLastChange: round(last.score.officialTotal - first.score.officialTotal, 3),
  };
}

function envelope<TFacts, TTool extends AnalysisToolName>(
  tool: TTool,
  facts: TFacts,
  citations: AnalysisCitation[],
  dataQuality: AnalysisToolEnvelope<TFacts, TTool>['dataQuality'],
): AnalysisToolEnvelope<TFacts, TTool> {
  return {
    tool,
    readOnly: true,
