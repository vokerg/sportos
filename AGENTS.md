...
- Daily summary transport now uses `features/daily/data-access/daily-api.service.ts` with contracts in `features/daily/model/daily.models.ts`. Focused legacy clients such as score breakdown and providers remain separate until their owning migrations; do not add feature methods to a global API service.
